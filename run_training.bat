@echo off
REM Training script for all three methods (Windows version)
REM Follows DR-LoRA experimental setup (Table 6)

setlocal

REM Configuration
set BASE_MODEL=allenai/OLMoE-1B-7B-0924
set OUTPUT_DIR=./results
set SEED=42

REM Training hyperparameters (matching DR-LoRA)
set TOTAL_STEPS=38000
set MICRO_BATCH=1
set GRAD_ACCUM=16
...
if errorlevel 1 (
    echo Standard LoRA training failed!
    exit /b 1
)
...
REM Method 3: Hierarchical Spawning
echo ==========================================
...
if errorlevel 1 (
    echo Hierarchical Spawning training failed!
    exit /b 1
)
